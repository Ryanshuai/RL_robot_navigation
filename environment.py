...
class ENV:
    def __init__(self):
        ...
        self.hunter_radius = 16
        self.escaper_radius = 16
    ...
    def _init_pos(self):
        ...
        self.total_frame = int(np.linalg.norm(self.hunter_pos - self.escaper_pos))
        self.last_hunter_pos = self.hunter_pos
        self.escaper_move = random.randint(1, 4)
    ...
    def frame_step(self, input_actions):
        ...
        #disreward = self.catch_dis/now_normdis - self.catch_dis/last_normdis
        #if disreward >= 0.5:
        #    disreward = 0.5
        #elif disreward <= -0.5:
        #    disreward = -0.5
        ...
        #if self.frame_step_counter == self.total_frame:
        if self.is_escaped():
            self.__init__()
            reward_hunter = 0
            terminal = True
        ...
        robot_state = np.array([self.escaper_pos[0], self.hunter_pos[0],self.escaper_pos[1], self.hunter_pos[1]])

        return robot_state, reward_hunter, terminal
    ...
    def move(self, input_actions):
        ...
        if self.hunter_pos[0] < 0+self.hunter_radius:
          self.hunter_pos[0] = 0+self.hunter_radius
          self.hunter_spd[0] = 0
        elif self.hunter_pos[0] > SCREEN_WHIDTH-self.hunter_radius:
          self.hunter_pos[0] = SCREEN_WHIDTH-self.hunter_radius
          self.hunter_spd[0] = 0

        if self.hunter_pos[1] < 0+self.hunter_radius:
          self.hunter_pos[1] = 0+self.hunter_radius
          self.hunter_spd[1] = 0
        elif self.hunter_pos[1] > SCREEN_HEIGHT-self.hunter_radius:
          self.hunter_pos[1] = SCREEN_HEIGHT-self.hunter_radius
          self.hunter_spd[1] = 0


        if self.escaper_move == 1:
            self.escaper_pos[1] -= self.escaper_radius / 4
        elif self.escaper_move == 2:
            self.escaper_pos[1] += self.escaper_radius / 4
        elif self.escaper_move == 3:
            self.escaper_pos[0] -= self.escaper_radius / 4
        elif self.escaper_move == 4:
            self.escaper_pos[0] += self.escaper_radius / 4
